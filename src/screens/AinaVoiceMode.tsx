      ...
      <div
        ...
        style={{
          background: isThinking
            ...
            : isActive
            ? 'radial-gradient(ellipse 100% 60% at 50% 50%, rgba(0,102,255,0.18) 0%, transparent 70%)'
            : 'radial-gradient(ellipse 100% 60% at 50% 50%, rgba(0,102,255,0.06) 0%, transparent 70%)',
          transition: 'background 600ms ease',
        }}
      />
      <svg width="280" height="80" viewBox="0 0 200 100" fill="none" style={{ overflow: 'visible' }}>
        <defs>
          <filter id="ainaWaveGlow">
            <feGaussianBlur stdDeviation="3" result="blur" />
            <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
          </filter>
          <linearGradient id="ainaWaveGrad" x1="0" y1="0" x2="1" y2="0">
            <stop offset="0%" stopColor="#0066FF" stopOpacity="0.6" />
            <stop offset="50%" stopColor="#AFC5FF" stopOpacity="1" />
            <stop offset="100%" stopColor="#0066FF" stopOpacity="0.6" />
          </linearGradient>
        </defs>
        <path
          d={WAVEFORM_PATH}
          stroke={isThinking ? 'rgba(175,197,255,0.3)' : '#AFC5FF'}
          strokeWidth="4"
          strokeOpacity={isActive ? 0.3 : 0.1}
          strokeLinecap="round"
          strokeLinejoin="round"
          fill="none"
          filter="url(#ainaWaveGlow)"
          style={{ transition: 'stroke-opacity 400ms ease' }}
        />
        <path
          d={WAVEFORM_PATH}
          stroke={isThinking ? 'rgba(175,197,255,0.5)' : 'url(#ainaWaveGrad)'}
          strokeWidth="2.5"
          strokeLinecap="round"
          strokeLinejoin="round"
          fill="none"
          style={{
            ...
            animation: isActive ? 'aina-voice-pulse 1.2s ease-in-out infinite' : isThinking ? 'aina-voice-idle 2s ease-in-out infinite' : 'none',
          }}
        />
        <circle
          cx="100"
          cy="50"
          r={isActive ? 4 : 2.5}
          fill={isActive ? '#AFC5FF' : 'rgba(175,197,255,0.3)'}
          style={{
            filter: isActive ? 'drop-shadow(0 0 6px rgba(0,102,255,0.8))' : 'none',
...
export default function AinaVoiceMode({ onEnd, onSwitchToText }: AinaVoiceModeProps) {
  ...
  const STATE_LABELS: Record<VoiceState, string> = {
    listening: 'Listening…',
    thinking:  'Aina is thinking…',
    speaking:  'Aina is speaking…',
    idle:      'Tap to speak',
  }
  ...
  return (
    <div className="flex flex-col bg-bg relative overflow-hidden" style={{ height: 785 }}>
      <style>{`
        @keyframes aina-voice-pulse {
          0%, 100% { transform: scaleY(1); }
          50% { transform: scaleY(1.18); }
        }
        @keyframes aina-voice-idle {
          0%, 100% { opacity: 0.55; }
          50% { opacity: 0.8; }
        }
      `}</style>

      {/* Full-bleed ambient background — blue merchant palette */}
      <div className="absolute inset-0 pointer-events-none">
        <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse 80% 60% at 50% 40%, rgba(0,50,160,0.14) 0%, transparent 70%)' }} />
        <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse 60% 40% at 50% 70%, rgba(0,102,255,0.07) 0%, transparent 60%)' }} />
      </div>
      ...
      <div className="flex items-center gap-3 px-5 pt-4 pb-2 relative z-10">
        ...
        <div className="flex-1 flex flex-col items-center">
          ...
          <p className="font-mono text-[10px]" style={{ color: 'rgba(0,153,255,0.7)' }}>{formatTime(duration)}</p>
        </div>
        <button
          onClick={() => setShowTranscript(p => !p)}
          className="flex items-center gap-1.5 h-9 px-3 rounded-full font-body text-xs font-semibold transition-all hover:bg-surface-hi"
          style={{ border: `1px solid ${showTranscript ? 'rgba(0,102,255,0.5)' : 'rgba(175,197,255,0.15)'}`, background: showTranscript ? 'rgba(0,102,255,0.14)' : 'transparent', color: showTranscript ? '#AFC5FF' : 'rgba(175,197,255,0.5)' }}>
          Transcript
        </button>
      </div>

      {/* Aina avatar — blue glow treatment (merchant) */}
      <div className="flex flex-col items-center justify-center flex-1 gap-8 relative z-10 px-6">
        <div className="flex flex-col items-center gap-5">
          <div className="relative flex items-center justify-center" style={{ width: 120, height: 120 }}>
            {(voiceState === 'listening' || voiceState === 'speaking') && (
              <div className="absolute inset-0 rounded-full"
                style={{ border: '1.5px solid rgba(0,102,255,0.25)', animation: 'ping 1.5s ease-out infinite' }} />
            )}
            <div className="absolute rounded-full"
              style={{ inset: 8, border: '1.5px solid rgba(0,102,255,0.2)', boxShadow: '0 0 20px rgba(0,102,255,0.15)' }} />
            <div
              className="relative z-10 w-20 h-20 rounded-full overflow-hidden"
              style={{
                boxShadow: voiceState === 'listening' || voiceState === 'speaking'
                  ? '0 0 40px rgba(0,102,255,0.55), 0 0 80px rgba(0,50,160,0.35)'
          ...
          <div className="flex flex-col items-center gap-1">
            <p className="font-display text-lg font-extrabold tracking-tight"
              style={{ background: 'linear-gradient(135deg, #0066FF 0%, #AFC5FF 100%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
              Aina
            </p>
            <p className="font-body text-sm transition-all duration-[300ms]"
              style={{ color: voiceState === 'thinking' ? 'rgba(175,197,255,0.5)' : 'rgba(0,153,255,0.9)' }}>
        ...
        {showTranscript && (
          <div
            ...
            style={{ bottom: 120, maxHeight: 280, background: 'rgba(5,18,60,0.97)', border: '1px solid rgba(0,102,255,0.2)', backdropFilter: 'blur(24px)' }}>
              ...
              {transcript.map(line => (
                <div key={line.id} className={`flex flex-col gap-0.5 ${line.role === 'user' ? 'items-end' : 'items-start'}`}>
                  <p className="font-body text-[9px] font-semibold uppercase tracking-wider"
                    style={{ color: line.role === 'aina' ? 'rgba(0,153,255,0.85)' : 'rgba(175,197,255,0.4)' }}>
                    {line.role === 'aina' ? 'Aina' : 'You'}
                  </p>
                  <div className="max-w-[85%] px-3 py-2 rounded-[12px]"
                    style={{
                      background: line.role === 'aina' ? 'rgba(0,30,90,0.85)' : 'rgba(0,102,255,0.2)',
                      border: line.role === 'aina' ? '1px solid rgba(0,102,255,0.25)' : 'none',
                    }}>
      ...
      <div className="px-8 pb-12 flex items-center justify-center gap-8 relative z-10">
        ...
        <button
          ...
          <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
            <path d="M3 4.5h12M3 9h8M3 13.5h10" stroke="rgba(175,197,255,0.65)" strokeWidth="1.3" strokeLinecap="round" />
          </svg>
        </button>
      </div>
    </div>
