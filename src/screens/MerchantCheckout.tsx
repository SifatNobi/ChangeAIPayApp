...
interface MerchantCheckoutProps {
  onBack?: () => void
  onSuccess?: (hasMerchantAccess: boolean) => void
  tierName?: TierName
  annualVolume?: number
}
...
export default function MerchantCheckout({
  ...
  // Checkout.tsx (personal). Web preview (Figma Make) keeps the simulated flow so design remains previewable.
  const handleSubscribe = async () => {
    if (!isRevenueCatNative()) {
      ...
      setTimeout(() => {
        setStep('success')
        setHasAccess(true)
        onSuccess?.(true) // simulated success for web preview
      }, 1500)
      return
    }
    setStep('processing')
    try {
      const result = await purchaseMerchantYearlyPackage(tier.name)
      const hasAccessResult = hasMerchantAccess(result.customerInfo)
      setHasAccess(hasAccessResult)
      setStep('success')
      onSuccess?.(hasAccessResult)
    } catch (error) {
        ...
        <button
          ...
          onClick={async () => {
            ...
            try {
              ...
              const hasAccessResult = hasMerchantAccess(customerInfo)
              setHasAccess(hasAccessResult)
              onSuccess?.(hasAccessResult)
            } catch (error) {
